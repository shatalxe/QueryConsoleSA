...
Функция ВыполнитьЗапрос() Экспорт
	...
	ИмяВременнойТаблицы 	= ТекущееЗадание.ВернутьВременнуюТаблицу;
	Отказ					= Ложь;
	...
	Если ТипЗнч(Параметры) = Тип("Структура") Тогда  		
		// Вызов "ВыполнитьЗапрос" из выполнения кода. Для упрощения программирования, в коде параметры передаются в виде структуры.
		Параметры = Новый Соответствие;
		Для Каждого КлючИЗначение Из ТекущееЗадание.Параметры Цикл 
			ОписаниеЗначения = Новый Структура("Значение, АдресЗначения, ТипПараметра"
				, КлючИЗначение.Значение
				, КлючИЗначение.Значение
				, ?(ЭтоАдресВременногоХранилища(КлючИЗначение.Значение), 1, 0));
			Параметры.Вставить(КлючИЗначение.Ключ, ОписаниеЗначения);
		КонецЦикла;
	КонецЕсли;
	...
	Для Каждого ОписаниеПараметра Из Запрос.НайтиПараметры() Цикл
		...
		ОписаниеЗначения = Параметры.Получить(Параметр); 
		...
		Если ЭтоАдресВременногоХранилища(ОписаниеЗначения.АдресЗначения) Тогда
			Значение = ПолучитьИзВременногоХранилища(ОписаниеЗначения.АдресЗначения);	
		КонецЕсли;
	...
	ИменаПереданныхВПараметрахВременныхТаблиц = Новый Массив; 
	ЗапросВременныеТаблицы = Новый Запрос;
	ЗапросВременныеТаблицы.МенеджерВременныхТаблиц = Запрос.МенеджерВременныхТаблиц;

	Для Каждого Параметр Из Параметры Цикл 
		ОписаниеЗначения = Параметр.Значение; 
		Если ОписаниеЗначения.ТипПараметра = 2 Тогда
			// Добавляем временную таблицу в менеджер  
			Имя = ОписаниеЗначения.Имя;  
			ЗапросВременныеТаблицы.Текст = ЗапросВременныеТаблицы.Текст + "ВЫБРАТЬ * ПОМЕСТИТЬ " + Имя + " ИЗ &" + Имя + " КАК " + Имя + ";" + Символы.ПС;  
			ИменаПереданныхВПараметрахВременныхТаблиц.Добавить(Имя);
			ЗапросВременныеТаблицы.Параметры.Вставить(Имя, ПолучитьИзВременногоХранилища(ОписаниеЗначения.АдресЗначения)); 
		КонецЕсли;
	КонецЦикла;  
	
	Если ИменаПереданныхВПараметрахВременныхТаблиц.Количество() Тогда  
		ЗапросВременныеТаблицы.Выполнить();
	КонецЕсли;
	...
	Для Каждого Таблица Из Запрос.МенеджерВременныхТаблиц.Таблицы Цикл  
		Если ИменаПереданныхВПараметрахВременныхТаблиц.Найти(Таблица.ПолноеИмя) = Неопределено Тогда // Не нужно возвращать имя ВТ, которое было передано в параметрах запроса.
...
Функция ВыполнитьКод() Экспорт
	...
	ПоместитьНеотображаемыеПараметрыВХранилище(ТекущееЗадание.Параметры, ТекущееЗадание.Форма.УникальныйИдентификатор);
...
Функция ВнешнееСоединениеВыполнитьЗапрос() Экспорт
	
	ТекстЗапроса 			= ТекущееЗадание.ТекстЗапроса;
	Параметры				= ТекущееЗадание.Параметры;
	Отказ					= Ложь;
	МассивОшибок			= Новый Массив;
	ПараметрыПодключения	= ТекущееЗадание.ПараметрыПодключенияКВнешнейБазе;
	
	НачатьЗамер(ТекстЗапроса);
	Соединение = ПолучитьПодключениеКВнешнемуИсточнику(ПараметрыПодключения);
		
	Если ПараметрыПодключения.ВариантСоединения = "V83COMConnector" Тогда
		РезультатЗапроса = ВыполнитьЗапросV83COMConnector(Соединение, ТекстЗапроса, Параметры, ТекущееЗадание.Пакет);  
		Результат = СоздатьПредставлениеРезультатаЗапросаV83COMConnector(Соединение, РезультатЗапроса);
	ИначеЕсли ПараметрыПодключения.ВариантСоединения = "ADODB" Тогда
		Результат = ВыполнитьЗапросADODB(Соединение, ТекстЗапроса);
	КонецЕсли;
...
Процедура ЗаполнитьДиаграммуЗамеров(Диаграмма) Экспорт
	...
	Запрос.УстановитьПараметр("Результаты", Результаты.Выгрузить());
	...
	Пока ВыборкаСтрокаВыполнения.Следующий() Цикл
		...
		СтрокаВыполнения = СокрЛП(ВыборкаСтрокаВыполнения.СтрокаВыполнения);
		
		Серия = Диаграмма.Серии.Добавить(Лев(СтрокаВыполнения, 50));
		Серия.Значение = СтрокаВыполнения;
...
Функция СведенияОВнешнейОбработке() Экспорт
    ...
    ПараметрыРегистрации.Вставить("Наименование", "Консоль запросов управляемые формы");
    ПараметрыРегистрации.Вставить("Версия", "1.2.21");
    ПараметрыРегистрации.Вставить("БезопасныйРежим", Ложь);
	ПараметрыРегистрации.Вставить("Информация", "Консоль запросов для управляемых форм с возможностью обработки результата запроса");
    ...
    НоваяКоманда.Представление = "Консоль запросов управляемые формы";
    НоваяКоманда.Идентификатор = "КонсольЗапросовУправляемыеФормыSA";
...
Функция СохранитьЗапрос(Запрос) Экспорт
	
	ОписаниеЗапроса = Новый Структура;
	ОписаниеЗапроса.Вставить("Текст", Запрос.Текст);
	ОписаниеЗапроса.Вставить("МенеджерВременныхТаблиц", Новый Соответствие);
	...
	Если Не Запрос.МенеджерВременныхТаблиц = Неопределено Тогда
		Для Каждого ОписаниеТаблицы Из Запрос.МенеджерВременныхТаблиц.Таблицы Цикл  		
			ОписаниеЗапроса.МенеджерВременныхТаблиц.Вставить(ОписаниеТаблицы.ПолноеИмя, ОписаниеТаблицы.ПолучитьДанные().Выгрузить());
		КонецЦикла;
	КонецЕсли;	
	
	Для Каждого КлючИЗначение Из Запрос.Параметры Цикл                        
		ОписаниеЗапроса.Параметры.Вставить(КлючИЗначение.Ключ, КлючИЗначение.Значение);		
	КонецЦикла;

	ХранилищеСистемныхНастроек.Сохранить(Метаданные().Имя, "ЗапросИзОтладки", ОписаниеЗапроса);
	
	Возврат "Настройки успешно сохранены";
...
Функция ЗначениеВТаблицуЗначений(Значение, Массив = Неопределено, ТекущаяКоллекция = Неопределено) Экспорт
	...
	Если ЭтоСписок(Значение) Тогда // Массив, список значений, РолиПользователя		
...
Функция ЗначениеВДеревоЗначений(Значение, Массив = Неопределено, ТекущаяКоллекция = Неопределено) Экспорт
	...
	Иначе	
		ЗначениеВТаблицуЗначений(Значение, Массив);		
	КонецЕсли; 
...
Функция СписокВТаблицуЗначений(МассивКВыводу, Значение) Экспорт
	...
	Если КоличествоЗначенийКоллекции(Значение) > 0 Тогда
		...
		ОписаниеТипаКолонки = Новый ОписаниеТипов(МассивТипов);
...
Процедура ДобавитьМассивЗначенийНаФому(Форма, ГруппаФормы, МассивЗначений, ДекорацияНетДанных = Неопределено) Экспорт
	...
	МассивТиповДляРасшифровки.Добавить(Тип("СоединениеИнформационнойБазы"));
	МассивТиповДляРасшифровки.Добавить(Тип("СеансИнформационнойБазы"));
...
Функция ДобавитьЭлементРезультатаНаФорму(Форма, ГруппаФормы, МассивЗначений, Значение, СчетчикТаблиц = 0, СчетчикПолей = 0)
	...
	Если ПоддерживаетсяВыводКоллекции(Значение) Тогда
		...
		ЭлементТаблица.УстановитьДействие("ПриАктивизацииСтроки", "Подключаемый_ПриАктивизацииСтроки");
		...
		Если ЗначениеЗаполнено(ВыводЗначений) И Значение = ВыводЗначений.Дерево Тогда
			// Команда УстановитьНовоеЗначение
			ИмяКоманды = ИмяРеквизита + "УстановитьНовоеЗначение";
			Заголовок = "Установить новое значение в виде выражения";
			ДобавитьКомандуФормы(Форма, ИмяКоманды, "Подключаемый_УстановитьНовоеЗначение", ЭлементТаблица, Заголовок, Заголовок);
			
			Элемент = Элементы.Добавить(ИмяКоманды, Тип("КнопкаФормы"), ЭлементТаблица.КоманднаяПанель);
			Элемент.Вид = ВидКнопкиФормы.ОбычнаяКнопка;
			Элемент.ИмяКоманды = ИмяКоманды;
			Элемент.Картинка = БиблиотекаКартинок.ПроизвольноеВыражение;
			Элемент.Отображение = ОтображениеКнопки.Картинка;
			// Команда КодДляЗаполненияОбъекта
			ИмяКоманды = ИмяРеквизита + "КодДляЗаполненияОбъекта";
			Заголовок = "Код для заполнения объекта";
			ДобавитьКомандуФормы(Форма, ИмяКоманды, "Подключаемый_КодДляЗаполненияОбъекта", ЭлементТаблица, Заголовок, Заголовок);
			
			Элемент = Элементы.Добавить(ИмяКоманды,	Тип("КнопкаФормы"), ЭлементТаблица.КоманднаяПанель);		
			Элемент.Вид = ВидКнопкиФормы.ОбычнаяКнопка;
			Элемент.ИмяКоманды = ИмяКоманды;
			Элемент.Картинка = БиблиотекаКартинок.Заменить;
			Элемент.Отображение = ОтображениеКнопки.Картинка;
		КонецЕсли;
...
Процедура ДобавитьНеотображаемыйЭлементФормы(Хранилище, ВладелецПоля, ИмяЭлемента, Значение)
	...
	Хранилище.Вставить(ИдентификаторЗначения, Значение);
		
КонецПроцедуры

Функция ДобавитьКомандуФормы(Форма, ИмяРеквизита, Действие, ИспользуемаяТаблица = Неопределено, Заголовок = "", Подсказка = "");
...
Функция ПолучитьПодключениеКВнешнемуИсточнику(ПараметрыПодключения) Экспорт
	...
	ИначеЕсли ПараметрыПодключения.ВариантСоединения = "ADODB" Тогда
		...
		Соединение.ConnectionString =
			?(ПустаяСтрока(ПараметрыПодключения.Драйвер), "driver={SQL Server};", "driver={" + ПараметрыПодключения.Драйвер + "};") +
			"server=" + ПараметрыПодключения.ИмяСервера + ";" +
			"uid=" + ПараметрыПодключения.Пользователь + ";" +
			"pwd=" + ПараметрыПодключения.Пароль + ";" +
			"database=" + ПараметрыПодключения.ИмяБазы + ";";
...
Функция ВыполнитьЗапросV83COMConnector(Соединение, ТекстЗапроса, ПараметрыЗапроса, Пакет = Ложь)
...
Функция СоздатьПредставлениеРезультатаЗапросаV83COMConnector(Соединение, РезультатЗапроса)
	
	Результат = Новый ТаблицаЗначений;
	РезультатТаблицаВнешняя = РезультатЗапроса.Выгрузить();
	
	Для Каждого Колонка Из РезультатТаблицаВнешняя.Columns Цикл
		Результат.Колонки.Добавить(Колонка.Name);
	КонецЦикла;
	
	Для Каждого ТекущаяСтрока Из РезультатТаблицаВнешняя Цикл
		
		НоваяСтрока = Результат.Добавить();
		Для Каждого Колонка Из Результат.Колонки Цикл
			НоваяСтрока[Колонка.Имя] = Соединение.String(ТекущаяСтрока[Колонка.Имя]);
		КонецЦикла;
...
// Возвращает результат выполнения команды ADODB
//
// Параметры:
...
//
Функция ВыполнитьЗапросADODB(Соединение, ТекстЗапроса) Экспорт
	...
	Если Выборка.BOF = Ложь Тогда
		...
		//Формируем колонки Таблицы значений
		Поля = Выборка.Fields.Count - 1;	
		Выборка.MoveFirst();
		Для сч = 0 по Поля Цикл
			ИмяКолонки = Выборка.Fields(сч).Name;
			ТаблицаЗначений.Колонки.Добавить(ИмяКолонки);
		КонецЦикла;	
		
		//Заполняем Таблицу значениц
		Выборка.MoveFirst();
		Пока НЕ Выборка.EOF() Цикл
			СтрокаТаблицыЗначений = ТаблицаЗначений.Добавить();
			Для сч = 0 по Поля Цикл
				СтрокаТаблицыЗначений[Выборка.Fields(сч).Name] = Выборка.Fields(сч).Value;
			КонецЦикла;
			Выборка.MoveNext();
		КонецЦикла;
	Иначе 
...
Процедура ПоместитьНеотображаемыеПараметрыВХранилище(Параметры, УникальныйИдентификатор) Экспорт
	
	Для Каждого КлючИЗначение Из Параметры Цикл		
		Если ТипЗнч(КлючИЗначение.Значение) = Тип("ТаблицаЗначений") Тогда  
			Параметры[КлючИЗначение.Ключ] = ПоместитьВоВременноеХранилище(КлючИЗначение.Значение, УникальныйИдентификатор);   
		ИначеЕсли ТипЗнч(КлючИЗначение.Значение) = Тип("Массив") Тогда 	 
			СписокЗначений = Новый СписокЗначений;
			СписокЗначений.ЗагрузитьЗначения(КлючИЗначение.Значение);
			Параметры[КлючИЗначение.Ключ] = СписокЗначений;
		КонецЕсли; 
	КонецЦикла;
...
Функция ПредставлениеЗначения(Значение) Экспорт
	...
	ИначеЕсли ТипЗначения = Тип("Структура") Тогда
		Представление = "Структура: " + Значение.Количество();
	ИначеЕсли ТипЗначения = Тип("РолиПользователя") Тогда
		Представление = "Роли пользователя: " + КоличествоЗначенийКоллекции(Значение);
	ИначеЕсли ТипЗначения = Тип("СоединениеИнформационнойБазы") 
		Или ТипЗначения = Тип("СеансИнформационнойБазы") Тогда  
		// Подменим значение на структуру, т.к. эти типы нельзя помещать во временное хранилище.
        Представление = Строка(Значение);
		НовоеЗначение = Новый Структура(
			"Пользователь, IPАдресКлиента, ИмяКомпьютера, НачалоСеанса, ИмяПриложения, НомерСеанса, НомерСоединения");
		ЗаполнитьЗначенияСвойств(НовоеЗначение, Значение); 
		Значение = НовоеЗначение; 
...
Функция ПредставлениеТипов(ОписаниеТипов)
	...
	Для Каждого ЭлементМассива Из МассивПредставлений Цикл
		Представление = ЭлементМассива + ?(МассивПредставлений.Найти(ЭлементМассива) = МассивПредставлений.Количество(), ";", "");
	КонецЦикла;
