...
Функция ВыполнитьЗапрос() Экспорт
	...
	ПереданныеВПараметрахВременныеТаблицы = Новый Массив;
	Для Каждого Параметр Из Параметры Цикл 
		...
		Если ОписаниеЗначения.ТипПараметра = 2 Тогда
			...
			ПереданныеВПараметрахВременныеТаблицы.Добавить(Имя);
			Запрос.Текст = "ВЫБРАТЬ * ПОМЕСТИТЬ " + Имя + " ИЗ &" + Имя + " КАК " + Имя + ";" + Символы.ПС;   
			Запрос.Параметры.Вставить(Имя, ПолучитьИзВременногоХранилища(ОписаниеЗначения.АдресЗначения)); 
		КонецЕсли;
	КонецЦикла;  
	Если ПереданныеВПараметрахВременныеТаблицы.Количество() Тогда
		Запрос.Выполнить();
	...
	Для Каждого Таблица Из Запрос.МенеджерВременныхТаблиц.Таблицы Цикл  
		Если ПереданныеВПараметрахВременныеТаблицы.Найти(Таблица.ПолноеИмя) = Неопределено Тогда // Не нужно возвращать имя ВТ, которое было передано в параметрах запроса.
...
Функция ВыполнитьКод() Экспорт
	...
	Для Каждого КлючИЗначение Из ТекущееЗадание.Параметры Цикл  
		// Неотображаемые типы, нельзя передать на клиент.
		Если ТипЗнч(КлючИЗначение.Значение) = Тип("ТаблицаЗначений") Тогда  
			ТекущееЗадание.Параметры[КлючИЗначение.Ключ] = ПоместитьВоВременноеХранилище(Результат, ТекущееЗадание.Форма.УникальныйИдентификатор);
		КонецЕсли;
	КонецЦикла;
...
Функция СведенияОВнешнейОбработке() Экспорт
    ...
    ПараметрыРегистрации.Вставить("Версия", "1.2.19");
