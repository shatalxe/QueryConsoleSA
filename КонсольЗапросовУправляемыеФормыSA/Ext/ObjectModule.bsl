...
Функция ВыполнитьЗапрос() Экспорт
	...
	Для Каждого ОписаниеПараметра Из Запрос.НайтиПараметры() Цикл
		...
		Если Не Параметры.Свойство(Параметр, Значение) Тогда
			Отказ = Истина;
			МассивОшибок.Добавить(ПодставитьПараметрыВСтроку(НСтр("ru = 'Не задано значение параметра %1'"),Параметр));
...
Функция ЗначениеВПроизвольнуюКоллекцию(Значение, Массив = Неопределено, ТекущаяКоллекция = Неопределено) Экспорт
	...
	ИначеЕсли ЭтоПримитивыныйТип(Значение)
		...
		Или ТипЗнч(Значение) = Тип("МоментВремени") Тогда
	...
	Иначе
		...
		Массив.Добавить("Не поддерживается вывод значения типа <" + ТипЗнч(Значение)+">");
...
Функция СписокВТаблицуЗначений(МассивКВыводу, Значение) Экспорт
	...
	ИмяКолонки = Строка(ТипЗнч(Значение));
...
Функция ДобавитьЭлементРезультатаНаФорму(Форма, ГруппаФормы, МассивЗначений, Значение, СчетчикТаблиц = 0, СчетчикПолей = 0)
	...
	Если ПоддерживаетсяВыводКоллекции(Значение) Тогда
		...
		ЭлементТаблица.УстановитьДействие("ПриАктивизацииСтроки", "Подключаемый_РезультатПриАктивизацииСтроки");
		...
		Если ТипЗначения = Тип("ДеревоЗначений") Тогда
			...
			ЭлементТаблица.УстановитьДействие("ПередРазворачиванием", "Подключаемый_РезультатПередРазворачиванием");
			// TODO Добавить команды Свернуть/Развернуть
		КонецЕсли;
	...
	ИначеЕсли ЭтоПримитивыныйТип(Значение)
		Или ТипЗначения = Тип("ДвоичныеДанные") Тогда 
...
Процедура ВывестиЗначенияСвойствНаФорму(Форма, Значение, ГруппаФормы)
	
	ВыводЗначений = НовыйВыводЗначенийСвойств(); 
	
	УдалитьДобавленныеРеквизитыФормы(Форма, ГруппаФормы);
	
	ДобавитьРеквизитыЗначенияНаФорму(Форма, ГруппаФормы, ВыводЗначений.МассивНовыхРеквизитов, ВыводЗначений.Дерево);
...
Функция НовыйВыводЗначенийСвойств()
	...
	Дерево.Колонки.Добавить("Тип", Новый ОписаниеТипов("Тип"));
...
Процедура ВыводЗначенийСвойствЗаполнитьДеревоСвойств(Форма, Узел, Значение, ВыводитьПредставления = Ложь)
	...
	Если ЭтоСсылочныйТип(Значение) Тогда
		...
		Для Каждого Реквизиты Из ИсточникРеквизитов Цикл
			Для Каждого Реквизит Из Реквизиты Цикл			
				ВыводЗначенийСвойствВывестиЗначение(Форма, НовыйУзел,  Реквизит.Имя, Значение[Реквизит.Имя], ВыводитьПредставления);
			КонецЦикла;
		КонецЦикла;
	...
	ИначеЕсли ЭтоСтруктураСоответствие(Значение) Тогда
		...
		//СтрокаЗначения = ВыводЗначенийСвойствВывестиЗначение(Форма, Строки, "Значение", Значение);
		Для Каждого КлючИЗначение Из Значение Цикл
	...
	ИначеЕсли ТипЗнч(Значение) = Тип("МоментВремени") Тогда
		...
		//СтрокаЗначения = ВыводЗначенийСвойствВывестиЗначение(Форма, Строки, "Значение", Значение, ВыводитьПредставления);
...
Функция ВыводЗначенийСвойствВывестиЗначение(Форма, Узел, Свойство, Значение, ВыводитьПредставления = Ложь)
	...
	Если ВыводитьПредставления Тогда
		...
		ДобавитьНеотображаемыйЭлементФормы(ХранилищеНеотображаемыхЗначений, НоваяСтрока, "Тип", ТипЗнч(Значение));
...
Функция ПредставлениеЗначения(Значение)
	...
	ИначеЕсли ТипЗначения = Тип("ОписаниеТипов") Тогда
		...
		Представление = Представление + Вычислить("СтрСоединить(МассивПредставлений,"";"")");
...
Функция ИтераторТаблицы(Значение)
	...
	ИначеЕсли ТипЗнч(Значение) = Тип("ТаблицаЗначений") Или ТипЗнч(Значение) = Тип("ДанныеФормыКоллекция") Тогда
		Возврат Значение;
	Иначе
