...
Функция ЗначениеВТабличныйДокумент(Значение) Экспорт
	...
	ИначеЕсли ЭтоПримитивыныйТип(Значение) // Строка, число, дата, булево
		...
		ЯчейкаСРасшифровкой = ВыводВТабличныйДокумент.МакетОформления.ПолучитьОбласть("R2C1"); 

		ЯчейкаСРасшифровкой.Параметры.Содержание = Строка(Значение);
		ЯчейкаСРасшифровкой.Параметры.Расшифровка = Значение;
		
		ВыводВТабличныйДокумент.ТабличныйДокумент.Вывести(ЯчейкаСРасшифровкой);
...
Функция НовыйВыводВТабличныйДокумент()
	...
	ВыводВТабличныйДокумент.Вставить("МакетОформления",ПолучитьМакет("ОформлениеРезультата"));
...
Функция ВыводВТабличныйДокументКонвертироватьСписок(ВыводВТабличныйДокумент, Значение)
	...
	Для Каждого ТекущийЭлемент Из Значение Цикл
		...
		ТабличныйДокумент.Вывести(ЗначениеВТабличныйДокумент(ТекущееЗначение),УровеньГруппировки-1);	
...
Функция ЗначениеВПроизвольнуюКоллекцию(Значение, Массив = Неопределено, ТекущаяКоллекция = Неопределено) Экспорт
	...
	ИначеЕсли ТипЗнч(Значение) = Тип("РезультатЗапроса") Тогда 
		...
		Массив.Добавить(Значение.Выгрузить()); // ОбходРезультатаЗапроса.ПоГруппировкам
	...
	ИначеЕсли ТипЗнч(Значение) = Тип("ОписаниеТипов") Тогда
		...
		Массив.Добавить(ПредставлениеОписанияТипов(Значение));
