...
Процедура Подключаемый_УстановитьНовоеЗначение(Команда)
	...
	ДанныеХранилищаЗначений = ПолучитьИзВременногоХранилища(АдресХранилищаЗначений);	
	
	ПараметрыФормы = Новый Структура;
	ПараметрыФормы.Вставить("Свойство", ТекущиеДанные.Свойство);
	ПараметрыФормы.Вставить("СсылкаНаИзменяемыйОбъект", ДанныеХранилищаЗначений.Получить(ИдентификаторЗначения));
	
	Оповещение = Новый ОписаниеОповещения("Подключаемый_УстановитьНовоеЗначениеЗавершение", ЭтаФорма);
	ОткрытьФорму("ВнешняяОбработка.КонсольЗапросовУправляемыеФормыSA.Форма.ВводВыражения", ПараметрыФормы, ЭтаФорма,,,, Оповещение, РежимОткрытияОкнаФормы.БлокироватьВесьИнтерфейс);
	...
	Для Сч = 0 По Колонки.Количество() - 1 Цикл
		...
		Если Сч < Колонки.Количество() -1 Тогда
			...
			Если СтрЗаканчиваетсяНа(Колонка.Имя, "Представление")
				И Колонка.Имя = СледующаяКолонка.Имя + "Представление" Тогда
				// TODO у колонки добавить тип значения
				//Значение = ХранилищеЗначений.Получить(Строка[СледующаяКолонка.Имя]);
				//МассивТипов = Новый Массив;
				//МассивТипов.Добавить(ТипЗнч(Значение));
				//СледующаяКолонка.ТипЗначения = Новый ОписаниеТипов(СледующаяКолонка.ТипЗначения, МассивТипов);
				Строка[СледующаяКолонка.Имя] = ХранилищеЗначений.Получить(Строка[СледующаяКолонка.Имя]);
				Продолжить;
			КонецЕсли;
