...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Для Каждого УзелКорня Из Корень Цикл
		...
		Для Каждого ОбъектМетаданных Из УзелКорня.Коллекция Цикл
			...
			//	
			//	Если Строка(ОбъектМетаданных.ПериодичностьРегистраСведений) <> "Непериодический"  Тогда
			//		
			//		СтрокаОбъекта = СтрокаКорня.ОбъектыМетаданных.Добавить();
			//		СтрокаОбъекта.ОбъектМетаданных = ОбъектМетаданных.Имя;
			//		СтрокаОбъекта.ТабличнаяЧасть = "СрезПервых"; //TODO переименовать в ВложеннаяТаблица и заполнять
			//		СтрокаОбъекта.ИсточникДанных = ОбъектМетаданных.Имя;
			//		СтрокаОбъекта.ПолноеИмяОбъектаМетаданных = ПолноеИмяОбъектаМетаданных;
			//		
			//	КонецЕсли;
...
Процедура СформироватьТекстЗапросаИЗакрыть(Поля)
	...
	ТекстЗапроса = СформироватьТекстЗапроса(Узел.УзелКореняМетаданных
...
&НаСервере
Функция СформироватьТекстЗапроса(УзелКореняМетаданных, ОбъектМетаданных, ТабличнаяЧасть, Поля)
	
	ИсточникДанных = ?(ТабличнаяЧасть = "", ОбъектМетаданных, ТабличнаяЧасть);
	ПолныйПуть = ?(ТабличнаяЧасть = "", УзелКореняМетаданных + "." + ОбъектМетаданных
		, УзелКореняМетаданных + "." + ОбъектМетаданных + "." + ТабличнаяЧасть);
