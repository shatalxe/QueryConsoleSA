...
Процедура ЗаполнитьСтруктуруЗапроса();
	...
	Если СтрНайти(Текст,"{") > 0 Тогда // TODO не нашел в СхемаЗапроса как с ними работать, пока оставил так. 
...
Процедура ПоказатьИндексы(ИмяТаблицы)
	...
	Если СтрНайти(ИмяТаблицы,".") = 0 Тогда // Это вложенный запрос или временная таблица. TODO запоминать это при заполнении дерева, для временных таблиц можно запоминать индексы.
		Возврат
	КонецЕсли;
