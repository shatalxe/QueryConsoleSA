...
&НаКлиенте
Процедура Подключаемый_РезультатПриАктивизацииСтроки(Элемент)

	//ЭлементКоманда = Элементы[Элемент.Имя + "ПоказатьЗначениеВОтдельномОкне"];
	//
	//ЭлементКоманда.Доступность = Ложь;
	//
	...
	//	ЭлементКоманда.Доступность = Истина;
	//КонецЕсли;
	
//	&НаСервере
//Функция ЕстьВозможностьПоказатьВОтдельномОкне(ИдетификаторЗначения) Экспорт
//	
//	ПолучитьИзВременногоХранилища(АдресХранилищаЗначений).Получить(ТекущиеДанные.Тип);
//		
//КонецФункции
...
Процедура ПоказатьЗначениеВОтдельномОкне(Форма, Команда, ТолькоСтроку = Истина) Экспорт
	...
	Если ВерсияПлатформы.Редакция = 83
		И ВерсияПлатформы.Версия > 12 Тогда 
...
Функция ВерсияПлатформы() Экспорт
	...
	ВерсияСтруктура.Вставить("Редакция", Описание.ПривестиЗначение(ВерсияМассив[0]) + Описание.ПривестиЗначение(ВерсияМассив[1])); 
