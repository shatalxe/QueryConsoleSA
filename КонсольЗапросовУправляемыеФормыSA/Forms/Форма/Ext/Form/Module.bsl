...
Процедура ДеревоНастроекПриАктивизацииСтроки(Элемент)
	...
	Если Элементы.ДеревоНастроек.ТекущаяСтрока <> Неопределено Тогда
		...
		ПутьКТекущейСтрокеДереваНастроек = ПолучитьПутьКСтрокеДерева(Элементы.ДеревоНастроек.ТекущиеДанные);
...
Процедура ДеревоНастроекСписокПараметровВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	Если Элемент.ТекущийЭлемент = Элементы.ДеревоНастроекСписокПараметровЗначение Тогда 
		Если ТекущиеДанные.ТипПараметра <> 0 Тогда 
			СтандартнаяОбработка = Ложь;
			ОткрытьФормуЗаполненияТаблицыЗначений(ТекущиеДанные);	
		КонецЕсли;
	КонецЕсли;
...
&НаКлиенте
Процедура ДеревоНастроекСписокПараметровЗначениеНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
...
Процедура ВыборРезультата(Форма, Элемент) Экспорт
	...
	Если Элемент.ТекущийЭлемент.ГиперссылкаЯчейки Тогда
		...
		СпособВывода = "ОткрытьФормуЗначенияВОтдельномОкне";
		...
		Если ЕстьСвойствоТип Тогда
			...
			ИначеЕсли Найти(Элемент.ТекущиеДанные.Тип, "Дата") = 1 
				Или Найти(Элемент.ТекущиеДанные.Тип, "Число") = 1
				Или Найти(Элемент.ТекущиеДанные.Тип, "Булево") = 1 Тогда 
				Значение = ПолучитьЗначениеИзВременногоХранилищаНаСервере(Форма.АдресХранилищаЗначений, ИдентификаторЗначения);
				СпособВывода = "ПоказатьВводЗначения";	
			ИначеЕсли Найти(Элемент.ТекущиеДанные.Тип, "Строка") = 1 Тогда
				Значение = ПолучитьЗначениеИзВременногоХранилищаНаСервере(Форма.АдресХранилищаЗначений, ИдентификаторЗначения);
				СпособВывода = "ОткрытьФормуЗначения";	
	...
	Если СпособВывода = "ОткрытьФормуЗначенияВОтдельномОкне" Тогда
	...
	ИначеЕсли СпособВывода = "ОткрытьФормуЗначения" Тогда 
		
		ПараметрыФормы = Новый Структура();
		ПараметрыФормы.Вставить("Значение", Значение);		
		ОткрытьФорму(ИмяОбработки() + ".Форма.ФормаЗначения", ПараметрыФормы, Форма,,,,, РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
...
Процедура СохранитьНастройкиПослеПроверкиСуществованияФайла(Существует, ДополнительныеПараметры) Экспорт
	...
	Если Существует Тогда  
		...
		Файл = Новый Файл(АдресНастроек);
		Файл.НачатьПолучениеВремениИзменения(Новый ОписаниеОповещения("СохранитьНастройкиПослеПроверкиСуществованияФайлаЗавершение", ЭтаФорма));
...
&НаКлиенте
Процедура СохранитьНастройкиПослеПроверкиСуществованияФайлаЗавершение(ВремяИзменения, ДополнительныеПараметры) Экспорт
	
	Если ВремяИзмененияНастроек <> ВремяИзменения Тогда   
		ОписаниеОповещения = Новый ОписаниеОповещения("ВопросОбъединитьНастройкиЗавершение", ЭтаФорма);
		ПоказатьВопрос(ОписаниеОповещения, "Настройки были изменены другим сеансом. Объединить?", РежимДиалогаВопрос.ДаНетОтмена);
		Возврат;
	КонецЕсли;
...
&НаКлиенте
Процедура ВопросОбъединитьНастройкиЗавершение(РезультатВопроса, ДополнительныеПараметры) Экспорт
	
	Если РезультатВопроса = КодВозвратаДиалога.Нет Тогда  
		СохранитьНастройкиПоАдресуНастроек();
	ИначеЕсли РезультатВопроса = КодВозвратаДиалога.Да Тогда
		
		ОбработкаОкончанияПомещения = Новый ОписаниеОповещения("ОбработчикОкончанияПомещенияНастроекНаСервер"
			, ЭтаФорма, Новый Структура("ОбъединениеНастроек", Истина));
...
Процедура СохранитьНастройкиПослеЗаписиФайла(Результат) Экспорт
	...
	Иначе
		файл = Новый Файл(АдресНастроек);
		файл.НачатьПолучениеВремениИзменения(Новый ОписаниеОповещения("СохранитьНастройкиПослеЗаписиФайлаЗавершение", ЭтаФорма));
	КонецЕсли;
	
КонецПроцедуры

&НаКлиенте
Процедура СохранитьНастройкиПослеЗаписиФайлаЗавершение(ВремяИзменения, ДополнительныеПараметры) Экспорт
	
	ВремяИзмененияНастроек = ВремяИзменения;
...
Процедура РазбитьЗапросПоЧастям(Команда)
	...
	ОткрытьФорму(ИмяОбработки() + ".Форма.АнализЗапросов", ПараметрыФормы,,,,, Оповещение
...
&НаКлиенте
Процедура ДобавитьИзОтладки(Команда) 
	
	Если ПустаяСтрока(СтрокаОтладки) Тогда
		...
		СтрокаОтладки = ПолучитьСтрокуОтладки(АдресВоВременномХранилище); 
		
	КонецЕсли;

	ОписаниеОповещения = Новый ОписаниеОповещения("ДобавитьИзОтладкиЗавершение", ЭтаФорма);   
	ПараметрыФормы = Новый Структура("СтрокаОтладки", СтрокаОтладки);
	ОткрытьФорму(ИмяОбработки() + ".Форма.ДобавитьИзОтладки", ПараметрыФормы,,,,, ОписаниеОповещения, РежимОткрытияОкнаФормы.БлокироватьВесьИнтерфейс);	
	
КонецПроцедуры

&НаКлиенте
Процедура ДобавитьИзОтладкиЗавершение(Результат, ДополнительныеПараметры) Экспорт
    ...
    ТекущиеДанные = Элементы.ДеревоНастроек.ТекущиеДанные;	
    ТекущиеДанные.ТекстЗапроса = Результат.Текст;
	
	Для Каждого Параметр Из Результат.Параметры Цикл 
		СтрокаПараметра = ТекущиеДанные.СписокПараметров.Добавить(); 
		СтрокаПараметра.Имя = Параметр.Ключ;
		Если ЭтоАдресВременногоХранилища(Параметр.Значение) Тогда 
			СтрокаПараметра.Значение = ПредставлениеЗначения(Параметр.Значение);  
			СтрокаПараметра.АдресЗначения = Параметр.Значение;
			СтрокаПараметра.ТипПараметра = 1;
  		Иначе
			СтрокаПараметра.Значение = Параметр.Значение;
			СтрокаПараметра.ТипПараметра = 0;   
		КонецЕсли;
	КонецЦикла; 
	Для Каждого Параметр Из Результат.МенеджерВременныхТаблиц Цикл 
		СтрокаПараметра = ТекущиеДанные.СписокПараметров.Добавить(); 
		СтрокаПараметра.Имя = Параметр.Ключ;  
		СтрокаПараметра.Значение = ПредставлениеЗначения(Параметр.Значение);  
		СтрокаПараметра.АдресЗначения = Параметр.Значение;
		СтрокаПараметра.ТипПараметра = 2;
	КонецЦикла;
...
Процедура Подключаемый_ВыполнитьЗапросВременнаяТаблица(Команда)
	...
	ИмяТаблицы = Прав(Команда.Имя, СтрДлина(Команда.Имя) - 3); // Удалим ключевой префикс ВТ_
	Задание = НовоеЗадание("ВыполнитьЗапрос", Истина, , ИмяТаблицы);
	ВыполнитьЗадание(Задание);
...
&НаКлиенте
Процедура ЗаменитьЗапрещенныеТипыТЗ(Команда)

	СписокПараметров = Элементы.ДеревоНастроек.ТекущиеДанные.СписокПараметров;
	ПараметрыУдалить = ЗаменитьЗапрещенныеТипыТЗНаСервере(СписокПараметров); 
	
	Для Каждого Параметр Из ПараметрыУдалить Цикл    
		Отбор = Новый Структура("Имя", Параметр);
		Элемент = Элементы.ДеревоНастроек.ТекущиеДанные.СписокПараметров.НайтиСтроки(Отбор)[0];  
		Элементы.ДеревоНастроек.ТекущиеДанные.СписокПараметров.Удалить(Элемент);
	КонецЦикла;	
...
Функция НовоеЗадание(ИмяМетода, ЭтоЗапрос = Ложь, Пакет = Ложь, ИмяВременнойТаблицы = "");
	...
	НовоеЗадание.Вставить("Параметры", ТекущиеПараметры(ЭтоЗапрос, НовоеЗадание.ЕстьОшибки));
	...
	НовоеЗадание.Вставить("ВернутьВременнуюТаблицу", ИмяВременнойТаблицы);
...
Процедура ВыполнитьЗадание(Задание)
	...
	Элементы.ДеревоНастроекСписокПараметров.Обновить(); // Платформа 8.3.22.1750. Если не обновить, то слетает условное оформление.
...
&НаКлиенте
Функция ТекущиеПараметры(ЭтоЗапрос, Отказ)

	ПроверитьДеревоНастроек();
	
	Если ЭтоЗапрос Тогда 
		ТекущиеПараметры = Новый Соответствие; // Для выполнения запроса необходимо определить тип параметра
	Иначе
		ТекущиеПараметры = Новый Структура(); // Из кода обращаемся к параметрам как к обычной струткуре.
	КонецЕсли;
	...
	Для Каждого СтрокаПараметра Из Элементы.ДеревоНастроек.ТекущиеДанные.СписокПараметров Цикл
		...
		Попытка     
			...
			Если ЭтоЗапрос Тогда     
				...
				ТекущиеПараметры.Вставить(СтрокаПараметра.Имя, СтруктураПараметра);
			Иначе
...
Функция ПолучитьНастройкиОбработки()
	...
	Настройки.Вставить("ДатаСохраненияНастроек", ТекущаяДата());
...
Процедура НачатьЗагрузкуНастроек(Знач Оповещение) Экспорт
	...
	Файл = Новый Файл(АдресНастроек);
	Файл.НачатьПроверкуСуществования(Новый ОписаниеОповещения("ЗагрузитьНастройкиПослеПроверкиСуществованияФайла"
		, ЭтаФорма, Оповещение));
...
Процедура ЗагрузитьНастройкиПослеПроверкиСуществованияФайла(Существует, ОповещениеПослеЗагрузкиНастроек) Экспорт
	...
	Если Существует Тогда  
		...
		Файл = Новый Файл(АдресНастроек);
		ВремяИзмененияНастроек = Неопределено;

		Файл.НачатьПолучениеВремениИзменения(Новый ОписаниеОповещения("ЗагрузитьНастройкиПослеПроверкиСуществованияФайлаЗавершение", ЭтаФорма, Новый Структура("ОповещениеПослеЗагрузкиНастроек", ОповещениеПослеЗагрузкиНастроек)));
...
&НаКлиенте
Процедура ЗагрузитьНастройкиПослеПроверкиСуществованияФайлаЗавершение(ВремяИзменения, ДополнительныеПараметры) Экспорт
	
	ОповещениеПослеЗагрузкиНастроек = ДополнительныеПараметры.ОповещениеПослеЗагрузкиНастроек;
...
Процедура ОбработчикОкончанияПомещенияНастроекНаСервер(Результат, Адрес, ВыбранноеИмяФайла, ОповещениеПослеЗагрузкиНастроек) Экспорт
	...
	Если Результат Тогда
		...
		Если ТипЗнч(ОповещениеПослеЗагрузкиНастроек) = Тип("Структура") 
			И ОповещениеПослеЗагрузкиНастроек.ОбъединениеНастроек Тогда 
			ЗагрузитьНастройкиИзФайла(Адрес, Истина);
		Иначе	
...
&НаСервере
Процедура ЗагрузитьНастройкиИзФайла(АдресВоВременномХранилище, РежимОбъединения = Ложь)
	...
	Попытка
		...
		Если РежимОбъединения Тогда 
			СравнитьОбъединитьНастройки(Дерево, ДеревоНастроек);	
		Иначе 
...
&НаСервере
Процедура СравнитьОбъединитьНастройки(УзелДобавления, УзелПриемника)
	
	ЭлементыПриемника = УзелПриемника.ПолучитьЭлементы();
	
	Для Каждого ТекущаяСтрока Из УзелДобавления.Строки Цикл
		
		СтрокиПриемника = НайтиСтрокуЭлементовДерева(ЭлементыПриемника, "ИмяНастройки", ТекущаяСтрока.ИмяНастройки);
		Если СтрокиПриемника.Количество() = 0 Тогда
			СтрокаПриемника = ЭлементыПриемника.Добавить();	
			ЗаполнитьЗначенияСвойств(СтрокаПриемника, ТекущаяСтрока,, "СписокПараметров"); // TODO добавить объекдинение параметров.
		Иначе    
			
			Если СтрокиПриемника.Количество() > 1 Тогда 
				// Попробуем подобрать по индексу
				Для Каждого Строка Из СтрокиПриемника Цикл
					Если УзелДобавления.Строки = ЭлементыПриемника.Индекс(Строка) Тогда   
						СтрокаПриемника = Строка;
						Прервать;
					КонецЕсли;
				КонецЦикла;  
				Если СтрокаПриемника = Неопределено Тогда
					// Однозначно не найти эту строку. Поэтому добавим новую
					СтрокаПриемника = ЭлементыПриемника.Добавить();	
					ЗаполнитьЗначенияСвойств(СтрокаПриемника, ТекущаяСтрока,, "СписокПараметров"); // TODO добавить объекдинение параметров.
				КонецЕсли;
				
			Иначе
				СтрокаПриемника = СтрокиПриемника[0];
			КонецЕсли;
			
			Если ТекущаяСтрока.ТекстЗапроса <> СтрокаПриемника.ТекстЗапроса
				Или ТекущаяСтрока.ПроизвольныйКод <> СтрокаПриемника.ПроизвольныйКод Тогда 
				// Значит добавим еще одну строку настроек и сохраним в неё.
				СтрокаПриемникаОбъединение = ЭлементыПриемника.Добавить();	
				ЗаполнитьЗначенияСвойств(СтрокаПриемникаОбъединение, ТекущаяСтрока,, "СписокПараметров");  // TODO добавить объекдинение параметров.	
				СтрокаПриемникаОбъединение.ИмяНастройки = СтрокаПриемникаОбъединение.ИмяНастройки + "_объединение"; 
			КонецЕсли;	
		КонецЕсли;
		
		СравнитьОбъединитьНастройки(ТекущаяСтрока, СтрокаПриемника);
...
Процедура ПослеЗагрузкиНастроек(Результат, ДополнительныеПараметры) Экспорт
	...
	ТекущаяСтрокаДеревоНастроек = ИдентификаторСтрокиДереваИзПути(ПутьКТекущейСтрокеДереваНастроек); 
	Элементы.ДеревоНастроек.ТекущаяСтрока = ТекущаяСтрокаДеревоНастроек;
...
&НаСервере
Функция НайтиСтрокуЭлементовДерева(ЭлементыДерева, ПолеПоиска, Значение)
	
	МассивНайденых = Новый Массив;   
	
	Для Каждого ТекущаяСтрока Из ЭлементыДерева Цикл  
		Если ТекущаяСтрока[ПолеПоиска] = Значение Тогда  
			МассивНайденых.Добавить(ТекущаяСтрока);
		КонецЕсли;	
	КонецЦикла;   
	
	Возврат МассивНайденых;
...
&НаКлиенте
Функция ПолучитьПутьКСтрокеДерева(ТекущиеДанные, Путь = "") 
	
	Родитель = ТекущиеДанные.ПолучитьРодителя();
	
	Если Родитель = Неопределено Тогда 
		ТекущаяКоллекция = ДеревоНастроек.ПолучитьЭлементы();
	Иначе           
		ПолучитьПутьКСтрокеДерева(Родитель, Путь);
	...
	ИндексСтроки = Формат(ИндексСтроки, "ЧН=0; ЧГ=");
	
	Путь = ?(Путь = "", ИндексСтроки, Путь + "," + ИндексСтроки);
...
&НаКлиенте
Функция ИдентификаторСтрокиДереваИзПути(Путь)
	
	МассивИндексов = РазложитьСтрокуВМассивПодстрок(Путь);
	...
	Для Каждого Индекс Из МассивИндексов Цикл  
		...
		Если ТекущаяКоллекция.Количество() <= Индекс Тогда 
			Возврат 0; // Путь не соответсвует настройкам. 
		Иначе
			ТекущиеДанные = ТекущаяКоллекция.Получить(Индекс);
		КонецЕсли;
		
		//Если Индекс = МассивИндексов.Количество() - 1 Тогда 
		//	Прервать;
		//КонецЕсли; 
		
		ТекущаяКоллекция = ТекущиеДанные.ПолучитьЭлементы();
...
Функция ПолучитьПараметрыИзЗапросаНаСервере(ТекстЗапроса, ТаблицаПараметров)
	...
	// Поиск параметров - таблиц значений	
	Для Каждого Источник Из ТаблицыЗначенийЗапроса(ТекстЗапроса) Цикл  
			
		НоваяТаблица = Новый ТаблицаЗначений;
		
		Для Каждого Поле Из Источник.ДоступныеПоля Цикл  
			НоваяТаблица.Колонки.Добавить(Поле, Новый ОписаниеТипов("Строка"));	
		КонецЦикла;  
		
		НайденныеПараметры.Добавить(СтруктураПараметра(
			Сред(Источник.ИмяТаблицы, 2)
...
Функция ТаблицыЗначенийЗапроса(ТекстЗапроса)
	...
	Для Каждого ПакетЗапросов Из СхемаЗапроса.ПакетЗапросов Цикл  
		...
		Для Каждого Оператор Из ПакетЗапросов.Операторы Цикл 
			Для Каждого Источник Из Оператор.Источники Цикл 
				...
				Если Не ТипЗнч(Источник) = Тип("ОписаниеВременнойТаблицыСхемыЗапроса") Тогда
					Продолжить;
				КонецЕсли;   
				
				Если Найти(Источник.ИмяТаблицы, "&") <> 1 Тогда
					Продолжить;
				КонецЕсли; 
				
				ДоступныеПоля = Новый Массив;
				Для Каждого Поле Из Источник.ДоступныеПоля Цикл  
					ДоступныеПоля.Добавить(Поле.Имя);
				КонецЦикла;
				
				Массив.Добавить(Новый Структура("ИмяТаблицы, Псевдоним, ДоступныеПоля", Источник.ИмяТаблицы, Источник.Псевдоним, ДоступныеПоля));
...
Функция ИменаВременныхТаблицЗапроса(ТекстЗапроса)
	...
	Для Каждого ПакетЗапросов Из СхемаЗапроса.ПакетЗапросов Цикл  
		...
		Если Не ПустаяСтрока(ПакетЗапросов.ТаблицаДляПомещения) Тогда 
			Массив.Добавить(ПакетЗапросов.ТаблицаДляПомещения);
		КонецЕсли;	
...
Функция	ДобавитьКомандыВыводаВременныхТаблиц(МассивВременныхТаблиц);
	...
	Для Каждого ИмяТаблицы Из МассивВременныхТаблиц Цикл
		...
		ИмяКоманды = "ВТ_" + ИмяТаблицы;
		НоваяКоманда = Команды.Добавить(ИмяКоманды);
		НоваяКоманда.Действие = "Подключаемый_ВыполнитьЗапросВременнаяТаблица";
		НоваяКоманда.Заголовок = ИмяТаблицы; 
		
		Элемент = Элементы.Добавить(ИмяКоманды, Тип("КнопкаФормы"), Элементы.ГруппаКнопокВременныеТаблицы);	
...
Процедура ПроверкаЗапросаНаСервере(ТекстЗапроса)
	...
	// Вывести список временных таблиц для возможности получения данных из них.	  
	ДобавитьКомандыВыводаВременныхТаблиц(ИменаВременныхТаблицЗапроса(ТекстЗапроса));
...
&НаКлиенте
Процедура ОткрытьФормуЗаполненияТаблицыЗначений(ТекущиеДанные); 
...
Функция ДополнитьПараметры(Форма, Приемник, Источник)
	...
	Для Каждого Параметр Из Источник Цикл
		...
		Если СтрокаПараметров.Количество() = 0 Тогда		
			...
			ЗаполнитьЗначенияСвойств(СтрокаПараметров, Параметр); 
			СтрокаПараметров.Значение = Параметр.ТипЗначения.ПривестиЗначение(СтрокаПараметров.Значение); // Для подстановки пустого значения нужного типа.
		...
		// Проверяем, нужно ли изменить значение.
		...
		Иначе            
			...
			ИначеЕсли Не ЗначениеЗаполнено(СтрокаПараметров.Значение) Тогда    
				СтрокаПараметров.Значение = Параметр.Значение;
			КонецЕсли;
		КонецЕсли;  
...
&НаКлиентеНаСервереБезКонтекста
Функция СтруктураПараметра(Имя = ""
	, ТипЗначения = Неопределено
	, Значение = Неопределено
	, ТипПараметра = Неопределено 
	, АдресЗначения = "")
	
	Структура = Новый Структура("Имя, ТипЗначения, Значение, ТипПараметра, АдресЗначения"
		, Имя, ТипЗначения, Значение, ТипПараметра, АдресЗначения); 
...
&НаСервере
Функция ЗаменитьЗапрещенныеТипыТЗНаСервере(СписокПараметров)
	...
	// TODO дописать корректирвоку таблицы, а не удаление параметра.
	// Попробовать объединить с методами в ФормаЗаполненияТаблицыЗначений.ПроверитьТипКолонки
	УдалитьПараметры = Новый Массив;
	
	Для Каждого Параметр Из СписокПараметров Цикл
		
		Если Не ЭтоАдресВременногоХранилища(Параметр.АдресЗначения) Тогда
			Продолжить;
		КонецЕсли;
		
		Таблица = ПолучитьИзВременногоХранилища(Параметр.АдресЗначения);
		НоваяТаблица = Новый ТаблицаЗначений;
		                 
		Для Каждого Колонка Из Таблица.Колонки Цикл
			Если Соответствие.Получить(Колонка.ТипЗначения) <> Неопределено Тогда  
				УдалитьПараметры.Добавить(Параметр.Имя);
				Перейти ~ВыходИзПроверки;
			КонецЕсли;						
		КонецЦикла;   
...
Функция ТекущиеПараметрыПодключения()
	...
	ТекущиеПараметры.Вставить("Драйвер"				, Драйвер);
...
&НаСервере
Функция ПолучитьСтрокуОтладки(АдресВоВременномХранилище) 
	
	ОбработкаОбъект = РеквизитФормыВЗначение("Объект"); 
	МетаданныеОбработки = ОбработкаОбъект.Метаданные();
	
	Если Метаданные.Обработки.Содержит(МетаданныеОбработки) Тогда
		СтрокаОтладки = "Обработки." + МетаданныеОбработки.Имя + ".СохранитьЗапрос(Запрос)";	
	Иначе  
		...
		СтрокаОтладки = "ВнешниеОбработки.Создать(""" + ИмяВременногоФайла + """,Ложь).СохранитьЗапрос(Запрос)";
		...
		//		ДополнительнаяОбработка = Справочники.ДополнительныеОтчетыИОбработки.НайтиПоРеквизиту("ИмяОбъекта", МетаданныеОбработки.Имя);
		//		Если ЗначениеЗаполнено(ДополнительнаяОбработка) Тогда
		//			Данные = ДополнительнаяОбработка.ХранилищеОбработки.Получить();
