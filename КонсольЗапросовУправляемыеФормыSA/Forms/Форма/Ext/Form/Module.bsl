...
Процедура Подключаемый_ВыборРезультата(Элемент)
	...
	Если Элемент.ТекущийЭлемент.ГиперссылкаЯчейки Тогда
		...
		ОткрытьФорму("ВнешняяОбработка.КонсольЗапросовУправляемыеФормыSA.Форма.РасшифровкаЯчеек",ПараметрыФормы,ЭтаФорма);
...
&НаКлиенте
Процедура Подключаемый_ПоказатьЗначениеВОтдельномОкне(Элемент)
	...
	ПараметрыФормы = Новый Структура();
	ПараметрыФормы.Вставить("Выражение","ВладелецФормы.Элементы."+ТекущийЭлемент.Имя+".ТекущиеДанные");
		
	ОткрытьФорму("ВнешняяОбработка.КонсольЗапросовУправляемыеФормыSA.Форма.ВычислениеВыражения",ПараметрыФормы,ЭтаФорма);
...
Процедура СохранитьНастройкиПослеПроверкиСуществованияФайла(Существует, ДополнительныеПараметры) Экспорт
	...
	Если Существует Тогда
		...
		Настройки.Записать(АдресНастроек); // TODO в веб клиенте ругается что метод Записать не обнаружен
...
Процедура ОткрытьКонструкторЗапроса(Команда)
	...
	ИначеЕсли АктивнаСтраницаПроизвольныйКод() Тогда
		ДобавитьУбратьСимволыВНачалоСтроки(Текст,"|",Истина);
		Текст = ЭлементНастроекПоТекущейСтранице().ВыделенныйТекст;
	КонецЕсли;
...
Процедура ПриЗакрытииКонструктораЗапроса(Результат, ДополнительныеПараметры) Экспорт
	...
	ИначеЕсли АктивнаСтраницаПроизвольныйКод() Тогда
		...
		ДобавитьУбратьСимволыВНачалоСтроки(Результат,"|");
		Элемент.ВыделенныйТекст = Результат;
...
Процедура ВыполнитьПроизвольныйКодНаКлиенте(Команда)
	...
	ВыполнитьЗаданиеНаКлиенте(ЭтотОбъект, ПараметрыВыполнения, Отказ); 
...
Процедура Закомментировать(Команда)
	...
	ДобавитьУбратьСимволыВНачалоТекстовогоДокумента("//")
...
Процедура Раскомментировать(Команда)
	...
	ДобавитьУбратьСимволыВНачалоТекстовогоДокумента("//",Истина)
...
Процедура УменьшитьОтступ(Команда)
	...
	ДобавитьУбратьСимволыВНачалоТекстовогоДокумента(Символы.Таб,Истина)
...
Процедура Подключаемый_ВыборРезультатаКоманда(Команда)
	...
	ИдентификаторЗначения = ДобавитьРеквизитВХранилищеЗначений(Команда.Имя); // Имя команды должно совпадать с именем реквизита.
...
&НаСервере
Функция ДобавитьРеквизитВХранилищеЗначений(ИмяРеквизита)
	
	ИдентификаторЗначения = Новый УникальныйИдентификатор;
	Хранилище = ПолучитьИзВременногоХранилища(АдресХранилищаЗначений);
	Хранилище.Вставить(ИдентификаторЗначения,РеквизитФормыВЗначение(ИмяРеквизита));
	ПоместитьВоВременноеХранилище(Хранилище,АдресХранилищаЗначений);
...
Процедура ВыполнитьЗаданиеНаСервере(ПараметрыВыполнения)
	...
	ОбработкаОбъект.КонтекстЗадания = ПараметрыВыполнения;
...
Функция ДобавитьНастройкуПоУмолчанию(ЭлементыДерева = Неопределено);
	...
	НоваяНастройка.ПроизвольныйКод = "Результат = ВыполнитьЗапрос().Выгрузить();";
...
Процедура ВывестиРезультат()
	...
	Если ТекущаяСтраница = Элементы.СтраницаРезультатаПоУмолчанию Тогда
		...
		МассивТаблиц = ОбработкаОбъект.РезультатВПроизвольнуюКоллекцию(Результат);  
	...
	ИначеЕсли ТекущаяСтраница = Элементы.СтраницаТабличныйДокумент Тогда
		...
		ТабличныйДокумент = ОбработкаОбъект.РезультатВТабличныйДокумент(Результат);
		
		ТабличныйДокументЗагружен = Истина;
		
	ИначеЕсли ТекущаяСтраница = Элементы.СтраницаТаблицаЗначений Тогда
		
		МассивТаблиц = ОбработкаОбъект.РезультатВТаблицуЗначений(Результат);
...
&НаКлиенте
Процедура ДобавитьУбратьСимволыВНачалоТекстовогоДокумента(НаборСимволов,РежимУдаления = Ложь)
...
&НаКлиенте
Процедура ДобавитьУбратьСимволыВНачалоСтроки(Строка,НаборСимволов,РежимУдаления = Ложь, Смещение = 0)
