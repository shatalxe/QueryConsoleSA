...
Функция ТекущиеПараметры(Отказ)
	...
	Для Каждого СтрокаПараметра Из Элементы.ДеревоНастроек.ТекущиеДанные.СписокПараметров Цикл
		Попытка     
			...
			Если СтрокаПараметра.НеотображаемыйТип <> 0 Тогда  
...
Процедура ОбработатьНеотображаемыеЗначенияТаблицыПараметров(УзелДерева, Режим)
	...
	Для Каждого Строка Из УзелДерева.Строки Цикл
		...
		Для Каждого Параметр Из Строка.СписокПараметров Цикл
			...
			Если Параметр.НеотображаемыйТип = 0 Тогда
				Продолжить;
			КонецЕсли;
			...
			Иначе
				...
				Параметр.Значение = "<Таблица значений>"; 
			КонецЕсли;
	...
	Попытка
		...
		Если Дерево.Строки.Количество() > 0 Тогда
			...
			Если Дерево.Строки[0].СписокПараметров.Колонки.Найти("НеотображаемыйТип") = Неопределено Тогда
				Совместимость_ДобавитьКолонкуВТаблицуПараметров(Дерево.Строки, "НеотображаемыйТип", Новый ОписаниеТипов("Число"));
...
Процедура ПолучитьПараметрыИзЗапросаНаСервере(ТекстЗапроса, ТаблицаПараметров, Модифицированность = Ложь)
	...
	Для Каждого Параметр Из Запрос.НайтиПараметры() Цикл
		НайденныеПараметры.Добавить(Новый Структура("Имя, ТипЗначения, Значение", Параметр.Имя, Параметр.ТипЗначения));
	КонецЦикла;
		...
		Для Каждого Оператор Из ПакетЗапросов.Операторы Цикл 
			Для Каждого Истончник Из Оператор.Источники Цикл 
				...
				Если Найти(Истончник.ИмяТаблицы, "&") = 1 Тогда
					...
					НайденныеПараметры.Добавить(Новый Структура("Имя, ТипЗначения, Значение", Сред(Истончник.ИмяТаблицы, 2), Новый ОписаниеТипов("ТаблицаЗначений"), НоваяТаблица));
	...
	ДополнитьПараметры(ЭтаФорма, ТаблицаПараметров, НайденныеПараметры);
...
Функция ДополнитьПараметры(Форма, Приемник, Источник)
	...
	Для Каждого Параметр Из Источник Цикл
		...
		Если ТипЗнч(СтрокаПараметров.Значение) = Тип("ТаблицаЗначений") Тогда  
			СтрокаПараметров.НеотображаемыйТип = 1;
			СтрокаПараметров.АдресЗначения = ПоместитьВоВременноеХранилище(Параметр.Значение, Форма.УникальныйИдентификатор);	 
			СтрокаПараметров.Значение = "<Таблица значений>"; 
		КонецЕсли;
...
&НаСервере
Процедура Совместимость_ДобавитьКолонкуВТаблицуПараметров(Строки, ИмяКолонки, Тип);
	
	Для Каждого Строка Из Строки Цикл		
		Строка.СписокПараметров.Колонки.Добавить(ИмяКолонки, Тип);		
		Совместимость_ДобавитьКолонкуВТаблицуПараметров(Строка.Строки, ИмяКолонки, Тип);	
	КонецЦикла;
