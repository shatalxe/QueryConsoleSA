...
Процедура ОткрытьРасположениеНастроек(Команда)
	...
	Если Не ПустаяСтрока(АдресНастроек) Тогда	
		ЗапуститьПриложение(КаталогНастроек());
	КонецЕсли;
...
Процедура КонструкторЗапроса(Команда)
	...
	#Если ТолстыйКлиентУправляемоеПриложение ИЛИ ТолстыйКлиентОбычноеПриложение Тогда
		...
		КонструкторЗапросаПриЗакрытии(КонструкторЗапроса.Текст, Новый Структура());
	#Иначе
		КонструкторЗапроса.Показать(Новый ОписаниеОповещения("КонструкторЗапросаПриЗакрытии",ЭтаФорма));
	#КонецЕсли
...
Процедура БыстрыйКонструктор(Команда)
	...
	Если Не (ПустаяСтрока(СтрокаВыполнения) Или СтрокаВыполнения = ТекстЗапросаПоУмолчанию()) Тогда
		Оповещение = Новый ОписаниеОповещения("БыстрыйКонструкторПослеОтветаНаВопрос", ЭтотОбъект);	
...
Функция КаталогНастроек()
	...
	ПоложениеФайла = Вычислить("СтрНайти(АдресНастроек, ""\"", НаправлениеПоиска.СКонца)");	
...
Процедура БыстрыйКонструкторОткрыть()
	...
	Оповещение = Новый ОписаниеОповещения("БыстрыйКонструкторЗакрытие", ЭтотОбъект);
