...
Процедура ВыборРезультата(Форма, Элемент) Экспорт
	...
	Если Элемент.ТекущийЭлемент.ГиперссылкаЯчейки Тогда
		...
		Если ЕстьСвойствоТип Тогда
			...
			Если Найти(Элемент.ТекущиеДанные.Тип, "Документ.") = 1
				Или Найти(Элемент.ТекущиеДанные.Тип, "Справочник.") = 1
				Или Найти(Элемент.ТекущиеДанные.Тип, "БизнесПроцесс.") = 1
				Или Найти(Элемент.ТекущиеДанные.Тип, "Задача.") = 1 
				Или Найти(Элемент.ТекущиеДанные.Тип, "ПланСчетов.") = 1
				Или Найти(Элемент.ТекущиеДанные.Тип, "ПланОбмена.") = 1
				Или Найти(Элемент.ТекущиеДанные.Тип, "ПланВидовХарактеристик.") = 1
				Или Найти(Элемент.ТекущиеДанные.Тип, "ПланВидовРасчета.") = 1 Тогда
...
Процедура ПриАктивизацииСтроки(Форма, Элемент) Экспорт
	...
	Если Элемент.Отображение = ОтображениеТаблицы.Дерево
		...
		Если Родитель <> Неопределено 
			...
			ЭлементУстановитьНовоеЗначение.Доступность = Родитель.Свойство = "Ссылка";
...
Процедура ВыполнитьЗапрос(Команда)
	...
	Задание = НовоеЗадание(Отказ, Истина);

	ВыполнитьЗадание(Задание, Отказ);	
...
Процедура Подключаемый_ВыполнитьЗапросВременнаяТаблица(Команда)
	...
	Задание = НовоеЗадание(Отказ, Истина, , Команда.Имя);
...
Процедура ВыполнитьПакет(Команда)
	...
	Задание = НовоеЗадание(Отказ, Истина, Истина);
...
Процедура ВыполнитьПроизвольныйКодНаКлиенте(Команда)
	...
	ВыполнитьЗаданиеНаКлиенте(ЭтаФорма, Задание, Отказ); 
...
&НаКлиенте
Функция НовоеЗадание(Отказ, ЭтоЗапрос = Ложь, Пакет = Ложь, ИмяВременнойТаблицы = "");
	
	НовоеЗадание = Новый Структура();	
	
	Если ЭтоЗапрос Тогда
		НовоеЗадание.Вставить("ИмяМетода", "ВыполнитьЗапрос");
		НовоеЗадание.Вставить("ТекстКода", Элементы.ДеревоНастроек.ТекущиеДанные.ПроизвольныйКод);
		НовоеЗадание.Вставить("ТекстЗапроса", СтрокаВыполнения(Отказ, Истина));
	Иначе
		НовоеЗадание.Вставить("ИмяМетода", "ВыполнитьКод");
		НовоеЗадание.Вставить("ТекстКода", СтрокаВыполнения(Отказ, Истина));
		НовоеЗадание.Вставить("ТекстЗапроса", Элементы.ДеревоНастроек.ТекущиеДанные.ТекстЗапроса);
	КонецЕсли;
		
	НовоеЗадание.Вставить("Параметры", ТекущиеПараметры(Отказ));
...
Функция ПолучитьЗначениеИзВременногоХранилищаНаСервере(Адрес, ИдентификаторЗначения, XMLСтрока = Ложь);
	
	Значение = ПолучитьИзВременногоХранилища(Адрес).Получить(ИдентификаторЗначения);
	
	Если XMLСтрока Тогда
